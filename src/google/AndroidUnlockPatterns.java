...
public class AndroidUnlockPatterns {
	public int numberOfPatterns(int m, int n) {
		...
		for (int i = m; i <= n; i++) {
			result += 4 * dfs(visited, skip, i - 1, 1);
			result += 4 * dfs(visited, skip, i - 1, 2);
			result += dfs(visited, skip, i - 1, 5);
		}
		return result;
	}
	private int dfs(boolean[] visited, int[][] skip, int remain, int cur) {
		...
		for (int i = 1; i <= 9; i++) {
			if (!visited[i] && (skip[cur][i] == 0 || visited[skip[cur][i]])) {
				result += dfs(visited, skip, remain - 1, i);
			}
		}
		visited[cur] = false;
		return result;
